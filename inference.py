...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for v in [PAC, GHOST0, GHOST1]:    
        lst = []
        for i in range(X_RANGE):
            for j in range(Y_RANGE):
                lst.append((i, j))
        variableDomainsDict[v] = lst
    g1 = []
    g2 = []
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST0]:
            dist = manhattanDistance(i, j)
            for n in range(1, MAX_NOISE + 1):
                tempdist1 = dist + n
                tempdist2 = dist - n
                if tempdist2 < 0:
                    tempdist2 = 0
                if tempdist1 not in g1:
                    g1.append(tempdist1)
                if tempdist2 not in g1:
                    g1.append(tempdist2)
        for k in variableDomainsDict[GHOST1]:
            dist = manhattanDistance(i, k)
            for m in range(1, MAX_NOISE + 1):
                tempdist1 = dist + m
                tempdist2 = dist - m
                if tempdist2 < 0:
                    tempdist2 = 0
                if tempdist1 not in g2:
                    g2.append(tempdist1)
                if tempdist2 not in g2:
                    g2.append(tempdist2)
    g1.sort()
    g2.sort()
    variableDomainsDict[OBS0] = g1
    variableDomainsDict[OBS1] = g2
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for i in eliminationOrder:
            ...
            if len(facjoined.unconditionedVariables()) > 1:
                evid.append(eliminate(facjoined, i))
        join = joinFactors(evid)
